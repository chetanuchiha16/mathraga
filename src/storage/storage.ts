...
import { AggregateStats, ProblemStats, SelectedSkill, SkillId, SkillProgress, TrainerState } from '@/types/game';
import { getLocalDateString, isSameDay, isYesterday } from '@/utils/dates';

export const STORAGE_VERSION = 1;
const STORAGE_KEY = 'hollow-math:trainer-state:v1';
...
function isProblemStatsMap(value: unknown): value is Record<string, ProblemStats> {
  return Boolean(value && typeof value === 'object' && !Array.isArray(value));
}

function normalizeSelectedSkill(value: unknown): SelectedSkill | null {
  if (value === 'all') return value;
  if (typeof value === 'string' && SKILL_IDS.includes(value as SkillId)) return value as SkillId;
  return null;
}

function normalizeAggregates(value: Partial<AggregateStats> | undefined): AggregateStats {
  return {
    totalAnswered: typeof value?.totalAnswered === 'number' ? value.totalAnswered : 0,
    totalCorrect: typeof value?.totalCorrect === 'number' ? value.totalCorrect : 0,
    recentAttempts: Array.isArray(value?.recentAttempts) ? value.recentAttempts.slice(0, 120) : [],
  };
}
...
function normalizeState(value: Partial<PersistedTrainerState>): TrainerState {
  ...
  const lastActiveDate = typeof value.streak?.lastActiveDate === 'string' ? value.streak.lastActiveDate : null;
  const streakStillCurrent =
    !lastActiveDate || isSameDay(lastActiveDate, today) || isYesterday(lastActiveDate, today);

  return {
    skillProgress: SKILL_IDS.reduce<Record<SkillId, SkillProgress>>((progressMap, skill) => {
      const existing = value.skillProgress?.[skill];
      progressMap[skill] = {
        level: Math.min(100, Math.max(1, existing?.level ?? defaults.skillProgress[skill].level)),
        attempts: existing?.attempts ?? 0,
        correct: existing?.correct ?? 0,
        recentAttempts: Array.isArray(existing?.recentAttempts)
          ? existing.recentAttempts.slice(0, 80)
          : [],
        masteredLevels: Array.isArray(existing?.masteredLevels) ? existing.masteredLevels.slice(-100) : [],
      };
      return progressMap;
    }, createDefaultSkillProgress()),
    problemStats: isProblemStatsMap(value.problemStats) ? value.problemStats : {},
    selectedSkill: normalizeSelectedSkill(value.selectedSkill),
    streak: {
      currentStreak: streakStillCurrent ? value.streak?.currentStreak ?? 0 : 0,
      ...
      dailyQuestionsAnswered: isSameDay(lastActiveDate, today)
        ...
        : 0,
      dailyGoal: Math.max(1, value.streak?.dailyGoal ?? DEFAULT_DAILY_GOAL),
    },
...
export async function loadTrainerState() {
  try {
    ...
    const parsed = JSON.parse(storedValue) as Partial<PersistedTrainerState>;
    if (parsed.version !== STORAGE_VERSION) return createDefaultTrainerState();
